	...
	void performAction(int action) {
		...
		switch(action){		
			...
			case 3:
				...
				break;
			/*case 4:
				// Middle and left rocket
				leftEngine.setBursting(true);
				rightEngine.setBursting(false);
				middleEngine.setBursting(true);
				break;
			case 5:
				// Middle and right rocket
				leftEngine.setBursting(false);
				rightEngine.setBursting(true);
				middleEngine.setBursting(true);
				break;
			case 6:
				// Left and right rocket
				leftEngine.setBursting(true);
				rightEngine.setBursting(true);
				middleEngine.setBursting(false);
				break;
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				/*System.out.println(Qtable.get(prev_stateaction));
				System.out.println(Ntable.get(prev_stateaction));
				System.out.println(alpha(Ntable.get(prev_stateaction)));
				System.out.println(getMaxActionQValue(new_state));*/											
				...
				//double val = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction)) * (previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state)) - Qtable.get(prev_stateaction);
				double val = previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state);
				//Qtable.put(new_state + previous_action, val);				
				Qtable.put(new_state + action, val);
